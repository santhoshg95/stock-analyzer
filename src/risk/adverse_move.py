...
class AdverseMoveRisk:
    ...
    def assess_intraday(intraday: pd.DataFrame, target_percent: float,
        ...
        for index in range(20, len(sessions) - horizon_days):
            ...
            if entry <= 0 or not (entry > float(row["SMA20"]) and float(row["RETURN5"]) > 0):
                continue
            target, barrier = entry * (1 + target_percent / 100), entry * (1 - adverse_percent / 100)
            ...
            overnight_gap = first_open <= barrier
            ...
            if not adverse_first:
                for _, candle in future.iterrows():
                    ...
                    if hit_target:
                        target_first = True
                        break
            worst = (float(future["Low"].min()) / entry - 1) * 100
        ...
        return {
            ...
            "data_resolution": "15_MINUTE", "horizon_days": horizon_days,
